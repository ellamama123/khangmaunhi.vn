				...
				<div class="row">
					<div class="col-xs-12">
						<div class="panel panel-info @if(isset($search['submit']) && $search['submit'] == CGlobal::status_show) act @endif">
							<form id="frmSearch" method="GET" action="" class="frmSearch" name="frmSearch">
								...
								<div class="panel-footer text-right">
									<a class="btn btn-default btn-sm" class="reset" href="{{route('admin.training')}}"><i class="fa fa-recycle"></i>Bỏ lọc</a>
									<a class="btn btn-danger btn-sm" href="{{FuncLib::getBaseUrl()}}admin/training/edit"><i class="ace-icon fa fa-plus-circle"></i>Thêm mới</a>
						...
						@if(sizeof($data) > 0)
							...
							@endif
							<br>
							<form id="formListItem" method="POST" action="{{FuncLib::getBaseUrl()}}admin/training/delete" class="formListItem" name="txtForm">
								<table class="table table-bordered table-hover">
									...
									<tr>
										...
										<th width="5%">Số buổi</th>
										<th width="10%">Ngày học</th>
										<th width="8%">Ngày bắt đầu</th>
										<th width="10%">Giá gốc(VND)</th>
										<th width="10%">Giá sau giảm(VND)</th>
									...
									@foreach($data as $k=>$item)
										<tr>
											...
											<td><a target="_blank" title="{{$item->statics_title}}" href="">{{$item['statics_title']}}</a></td>
											<td>
												@if(isset($arrCate[$item['statics_catid']])) {{ $arrCate[$item['statics_catid']] }} @endif
											</td>
											<td class="text-center">{{$item['statics_num']}}</td>
											<td>{{$item['statics_time']}}</td>
											<td class="text-center">{{($item['statics_date_start'] > 0) ? date('d-m-y', $item['statics_date_start']) : ''}}</td>
											<td class="text-bold">{{FuncLib::numberFormat($item['statics_price_normal'])}}</td>
											<td class="text-bold">{{FuncLib::numberFormat($item['statics_price_discount'])}}</td>

											<td class="text-center">{{$item['statics_order_no']}}</td>
											...
											<td>
												<a href="{{FuncLib::getBaseUrl()}}admin/training/edit/{{$item['statics_id']}}" title="Cập nhật">
