            ...
            <div class="row">
                @if(isset($dataCate->category_id) && $dataCate->category_image == '' && isset($data) && $data->count() > 1)
                    <div class="line postViewHead mgt25">
                        <div class="col-md-12 box-title-step cat-left">
                            <h1>
                ...
                @if(isset($data))
                    @if($data->count() > 1)
                        <div class="line mgt20 listPostTraining">
                            @foreach($data as $item)
                                <div class="col-lg-4 col-md-4 col-sm-12 item-post-training">
                                    <div class="item">
                                        <a title="{{$item->statics_title}}" href="{{FuncLib::buildLinkDetailNews($item->statics_id, $item->statics_title)}}">
                                            <div class="thumbI">
                                                @if($item->statics_image != '')
                                                    <img alt="{{$item->statics_title}}" src="{{ThumbImg::thumbBaseNormal(CGlobal::FOLDER_TRAINING, $item->statics_id, $item->statics_image, 600, 600, '', true, true)}}">
                                                @endif
                                            </div>
                                            ...
                                            <div class="boxPriceKhoaHocNoiBat">
                                                @if($item->statics_price_normal > 0)
                                                    <span class="price-normal">Giá gốc:  {{FuncLib::numberFormat($item->statics_price_normal)}}đ</span>
                                                @endif
                                ...
                                @foreach($data as $item)
                                    <div class="panelBox">
                                        <div class="training-title-view">
                                            {!!stripslashes($item['statics_title'])!!}
                                        </div>

                                        @if($item['statics_intro'] != '')
                                        <div class="training-intro-view mgt15">
                                            {!!stripslashes($item['statics_intro'])!!}
                                        </div>
                                        @endif

                                        @if($item['statics_content'] != '')
